...
def main():
    """
    Run four otherwise-identical OOS portfolio backtests while varying
    ONLY the trailing ATR stop. The fixed ATR stop remains 3.0x.
    """
    global STOP_ATR_MULTIPLIER
    global TRAILING_ATR_MULTIPLIER

    STOP_ATR_MULTIPLIER = 3.0
    trailing_values = [2.0, 2.5, 3.0, 3.5]
    ...
    for trailing_value in trailing_values:
        TRAILING_ATR_MULTIPLIER = trailing_value

        print()
        print("-" * 50)
        print(
            f"TEST: FIXED ATR STOP = 3.0x | "
            f"TRAILING ATR STOP = {trailing_value:.1f}x"
        )
        print("-" * 50)

        summary, equity_df, trades_df = run_portfolio_backtest(
            all_predictions
        )

        summary_row = dict(summary)
        summary_row["FixedStopATR"] = STOP_ATR_MULTIPLIER
        summary_row["TrailingStopATR"] = trailing_value
        summaries.append(summary_row)

        output_dir = Path(
            "portfolio_trailing_stop_sensitivity_results"
        ) / f"fixed_3.0_trailing_{trailing_value:.1f}"
        ...
        print(f"Fixed stop     : {STOP_ATR_MULTIPLIER:.1f}x")
        print(f"Trailing stop  : {trailing_value:.1f}x")
