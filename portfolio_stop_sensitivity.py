...
# ============================================================
# STOP SENSITIVITY TEST
# ============================================================

STOP_ATR_MULTIPLIERS = [2.0, 2.5, 3.0]


def main():
    global STOP_ATR_MULTIPLIER

    output_dir = Path(
        "portfolio_stop_sensitivity_results"
    )
    output_dir.mkdir(
        exist_ok=True
    )
    ...
    print(
        "ONE TIME ONLY - SHARED BY ALL STOP TESTS"
    )
    print(
        "=================================================="
    )
    ...
    # --------------------------------------------------------
    # Generate OOS predictions once. Every stop configuration
    # uses exactly the same predictions, assets, thresholds,
    # fees, slippage and portfolio logic.
    # --------------------------------------------------------
    ...
    for ticker in ASSETS:
        try:
            ...
            predictions = generate_oos_predictions(
                df
            )
    ...
    print(
        "RUNNING STOP SENSITIVITY TEST"
    )
    print(
        "Fixed ATR stop: 2.0x / 2.5x / 3.0x"
    )
    print(
        "Trailing ATR remains fixed at 2.5x"
    )
    print(
        "=================================================="
    )
    ...
    # --------------------------------------------------------
    # Run the exact same portfolio engine three times.
    # Only STOP_ATR_MULTIPLIER changes.
    # --------------------------------------------------------

    for multiplier in STOP_ATR_MULTIPLIERS:
        ...
        print(
            f"TEST: FIXED ATR STOP = {multiplier:.1f}x"
        )
        print(
            "--------------------------------------------------"
        )
        ...
        results.append(
            {
                "StopATR": multiplier,
                "TrailingATR": TRAILING_ATR_MULTIPLIER,
                **summary,
            }
        )

        label = f"stop_{multiplier:.1f}atr".replace(
            ".",
            "_",
        )
